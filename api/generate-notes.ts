...
Return ONLY this JSON structure:
{
  ...
  "myNotes": "${
    notesSource === 'transcript'
      ? `Write this section as a clear, well-structured explainer — the way a sharp teacher writes notes to actually make a student understand, not a compressed diary. Use markdown structure: short ## sub-headings for each topic shift in the video, bullet points (-) for lists of related points, and a short paragraph (2-4 sentences) wherever a concept needs connected explanation rather than a fragment. Cover the ENTIRE video start to end, following its own order — do NOT stop after the intro; if it covers many sub-topics, give each its own sub-heading with at least 2-4 lines under it, so the notes represent the whole video, not just the first part. Paraphrase in your own simple words, never copy the transcript's wording. Bold (**text**) the 3-8 key terms that matter most. Use whatever mix of headings/bullets/short-paragraphs best explains each part — don't force every section into the same shape. Add 1-2 short 'Q:' (question to revisit) or 'Note:' (gotcha/reminder) callouts where genuinely useful, not as decoration. End with a short '## Quick Recap' (2-4 bullet points) covering the video's full span. No filler intro like 'In this video...' — start directly with the content. Every line must be separated by an actual newline (\\n) — never return this as one continuous paragraph.`
      : `No transcript available for this video, so write general well-structured study notes on the topic itself — same style as above (## sub-headings, bullets, bolded key terms, short paragraphs where needed, real newlines between lines) — rather than pretending they're from a specific video.`
  }",
